...
import type { GameState, CharacterMapping, LevelSet, ProgressInfo } from '../types';
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const totalQuestions = contentGenerator.calculateQuestionsPerLevel(state.currentCharacterIndex);
    dispatch({ 
  ...
  useEffect(() => {
    if (!state.currentContentItem && state.currentContentIndex < state.currentLevelTotalQuestions) {
      generateNextContentItem();
    }
  }, [state.currentContentItem, state.currentContentIndex, state.currentLevelTotalQuestions]);
  ...
  const getCurrentLevelSet = (): LevelSet => {
    return LEVEL_SETS.find(set => set.id === state.currentLevelSetId) || LEVEL_SETS[0];
  };

  const getCurrentCharacter = (): CharacterMapping | null => {
    const currentLevelSet = getCurrentLevelSet();
    if (!currentLevelSet || currentLevelSet.characterMappings.length === 0) return null;
    return currentLevelSet.characterMappings[state.currentCharacterIndex] || null;
  };
  ...
  const generateNextContentItem = () => {
    ...
    const previousCharacters = currentLevelSet.characterMappings.slice(0, state.currentCharacterIndex);
  ...
  const getProgressInfo = (): ProgressInfo => {
    ...
    return {
      ...
      overallProgress: {
        ...
        total: LEVEL_SETS.reduce((total, set) => total + Math.max(set.characterMappings.length, 1), 0),
      },
    };
  };
  ...
  const submitAnswer = (answer: string): boolean => {
    ...
    const validation = contentGenerator.validateContentItem(contentItem, answer);
  ...
  const advanceToNext = () => {
    ...
    if (state.currentContentIndex >= state.currentLevelTotalQuestions - 1) {
      ...
      if (state.currentCharacterIndex >= currentLevelSet.characterMappings.length - 1) {
        // Current level set complete
        const currentSetIndex = LEVEL_SETS.findIndex(set => set.id === state.currentLevelSetId);
        ...
        const nextCharacter = currentLevelSet.characterMappings[state.currentCharacterIndex + 1];
        if (nextCharacter) {
          dispatch({ type: 'ADVANCE_LEVEL', payload: { newCharacter: nextCharacter } });
        }
      }
    } else {
      // Advance to next content item
