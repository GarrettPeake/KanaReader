...
describe("gameReducer", () => {
  ...
  test("should advance level", () => {
    const action = {
      type: "ADVANCE_LEVEL" as const,
      payload: { newCharacter: mockCharacter },
    };
    const result = gameReducer(initialGameState, action);
  ...
  test("should advance level set", () => {
    const action = {
      type: "ADVANCE_LEVEL_SET" as const,
      payload: { levelSetId: "katakana" },
    };
    const result = gameReducer(initialGameState, action);
  ...
  test("should handle transition visibility", () => {
    const showAction = {
      type: "SHOW_LEVEL_TRANSITION" as const,
      payload: true,
    };
    const hideAction = {
      type: "SHOW_LEVEL_TRANSITION" as const,
