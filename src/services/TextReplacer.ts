...
export class TextReplacer {
  static replaceText(sentence: string, unlockedCharacters: CharacterMapping[]): string {
    ...
    const kanjiMappings = unlockedCharacters.filter(mapping => mapping.id.startsWith('kanji_'));
    const otherMappings = unlockedCharacters.filter(mapping => !mapping.id.startsWith('kanji_'));
    
    // Process kanji with word boundary matching first
    const kanjiReplacements = kanjiMappings.flatMap(mapping => 
    ...
    const otherReplacements = otherMappings.flatMap(mapping => 
  ...
  static getOriginalText(sentence: string): string {
    // This method returns the original sentence without any character replacements
    // Used for validation and showing the expected input
    return sentence;
  }

  static hasTargetRomanizations(sentence: string, romanizations: string[], isKanji: boolean = false, translations?: string[]): boolean {
