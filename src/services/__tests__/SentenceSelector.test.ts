...
describe('SentenceSelector', () => {
  test('should select tutorial sentences', () => {
    const result = SentenceSelector.selectTutorialSentences(mockSentenceBank, 3);
    expect(result).toHaveLength(3);
    expect(mockSentenceBank.short).toEqual(expect.arrayContaining(result));
  });
  ...
  test('should select practice sentences with unlocked characters', () => {
    const unlockedChars = [mockCharacter];
    const result = SentenceSelector.selectPracticeSentences(
      mockSentenceBank,
      unlockedChars,
      2
    );
