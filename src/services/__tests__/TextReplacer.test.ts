...
const mockCharacters: CharacterMapping[] = [
  ...
  { id: 'ka_ka', character: 'カ', romanizations: ['ka', 'ca'], pronunciation: 'kah' },
  { id: 'ka_chi', character: 'チ', romanizations: ['chi'], pronunciation: 'chee' },
];

describe('TextReplacer', () => {
  test('should replace single characters correctly', () => {
    const result = TextReplacer.replaceText('I have a cat', [mockCharacters[0]]);
  ...
  test('should handle multiple romanizations per character', () => {
    const result = TextReplacer.replaceText('I can eat cake', [mockCharacters[2]]);
    expect(result).toBe('I カn eat カke'); // 'ca' gets replaced with 'カ'
  });

  test('should prioritize longer romanizations', () => {
    const result = TextReplacer.replaceText('The chicken is here', [mockCharacters[3]]);
  ...
  test('should handle multiple characters', () => {
    const result = TextReplacer.replaceText('I have a big cat', [mockCharacters[0], mockCharacters[1]]);
  ...
  test('should handle edge cases with punctuation', () => {
    const result = TextReplacer.replaceText('Hello, I am happy!', [mockCharacters[0], mockCharacters[1]]);
  ...
  test('should check if sentence contains target romanizations', () => {
    expect(TextReplacer.hasTargetRomanizations('I have a cat', ['a'])).toBe(true);
    expect(TextReplacer.hasTargetRomanizations('I have a cat', ['x'])).toBe(false);
    expect(TextReplacer.hasTargetRomanizations('I can do it', ['ca'])).toBe(true); // 'ca' in 'can'
    expect(TextReplacer.hasTargetRomanizations('Apple pie', ['a'])).toBe(true); // 'a' in 'Apple'
  });

  test('should return original text unchanged', () => {
    const original = 'This is a test sentence';
    expect(TextReplacer.getOriginalText(original)).toBe(original);
  });
});